...
یک پایگاه داده‌ی درون‌حافظه‌ای (In-Memory) است که به عنوان یک مخزن کلید-مقدار (Key-Value Store) استفاده می‌شود. این پایگاه داده به دلیل سرعت بالا و عملکرد عالی، معمولاً برای ذخیره‌سازی داده‌های موقت، کش (Cache)، صف‌های پیام (Message Queues) و سناریوهای دیگر که نیاز به دسترسی سریع به داده‌ها دارند، استفاده می‌شود.
